(() => {
      ...
      visibleColors.forEach(
        (colorClass, k) => {
          ...
          if (!isGoal) {  
            ...
            const r = isPortrait ? (count <= 6 ? 10 : 15) : (count <= 6 ? 20 : 26);
  
            s.style.transform = `translate( ${Math.cos(angle) * r}px,${Math.sin(angle) * r}px)`;  
          } else {  
            if (isPortrait) {  
            ...
            } else {const col = k % 2 === 0 ? -14: 14;  
              const rowHeight = Math.min(20, 180 / Math.ceil(count / 2));  
              const row = (Math.floor(k / 2) - (count / 4)) * rowHeight;
  
              const offsetX = col + Math.sin(k * 3) * 3;  
              const offsetY = row + Math.cos(k * 2) * 3;  
              s.style.transform = `translate(${offsetX}px, ${offsetY}px)`;
            }
          }  
    ...
    function nextIndex(index) {return (index + 1) % 14;}    
    /*
      Kalaha / Sunkaでは相手のゴールを飛ばす。  
      Basicでは 全14マスに入れる。
    */  
    function nextPlayableIndex(index, player) {  
      let next = nextIndex(index);  
      if ((isKalaha() || isSunka()) && next === opponentGoal(player)) {  
        next =  nextIndex(next);
      }  
      return next;
    }  
    ...
    function legalMoves(player, state = board) {  
      return playerPits(player).filter(i => state[i] > 0);
    }  
    ...
    async function collectRemainingStones(animate = true) {
  
      if (isBasic()) { return;}
  
      const humanPits = playerPits('human');  
      const cpuPits = playerPits('cpu');    
      //YOUの残り石  
      let humanRemaining = [];  
      humanPits.forEach(i => {  
        humanRemaining.push(...boardStones[i]);  
        boardStones[i] = [];
      });
  
      //CPUの残り石  
      let cpuRemaining = [];  
      cpuPits.forEach(i => {cpuRemaining.push(...boardStones[i]);  
        boardStones[i] = [];
      });
    
      //ゴールへ移動  
      boardStones[6].push(...humanRemaining);  
      boardStones[13].push(...cpuRemaining);  
      render();
  
      if (animate && (humanRemaining.length > 0 || cpuRemaining.length > 0)) {
        await sleep(300);
      }
    }  
    ...
    async function finishGame() {
      ...
      if (isBasic()) {  
        /*
          Basicは「先に自分の陣地を空にした人が勝ち」  
          humanEmpty = YOUの勝ち
          cpuEmpty   = CPUの勝ち
        */
  
        const humanEmpty = playerPits('human').every(i => board[i] === 0);  
        const cpuEmpty = playerPits('cpu').every(i => board[i] === 0); 
        ...
        //Kalaha / Sunka  最終ゴール数で勝敗 
  
        const humanScore = boardStones[6].length;  
        const cpuScore = boardStones[13].length;  
      ...
      return true;
       ...
       /* =========================================================
       MOVE RESULT
       performMoveがもう一度自分のターンか」をここに返す。  
       true: 同じプレイヤー  
       false: 相手へ
       ========================================================= */  
    ...
    async function performMove(start, player) {  
      moveAgain = false;  
      ...
      // 石を1個ずつ置く  
      while (hand.length > 0) {  
        pos = nextPlayableIndex(pos, player);  
  
        //最後に置く前の状態を保存。  
        const wasOccupied = boardStones[pos].length > 0;  
      ...
      if (isKalaha()) {  
        ...
        //自分の陣地の空ポケットで終わった→ 横取り  
        if (isOwnPit(pos, player) && boardStones[pos].length === 1) {  
          const opposite = oppositePit(pos);  
  
          //自分が今置いた1個 + 対面の相手の石全部  
          //相手が0個でも自分の1個は必ずゴールへ。  
          const captured = [...boardStones[opposite]];  
          boardStones[opposite] = [];    
          const taken = [...boardStones[pos]];  
          boardStones[pos] = []; 
    ...
    /* =========================================================
       SUNKA MOVE
       ========================================================= */  
      async function performSunkaMove(start, player) {
        moveAgain = false;      
        let hand = [...boardStones[start]];
        boardStones[start] = [];      
        render();      
        ...
        while (hand.length > 0) {
          ...
          // 石を置く「前」に、そのポケットに石があったか記録
          const hadStoneBefore = boardStones[pos].length > 0;
          ...
          // 最後の場所が自分のゴールのとき      
          if (pos === ownGoal(player)) {moveAgain = true; return;}
      
          // Sunka連鎖  最後に置いたポケットに「置く前から」石があったとき      
          if (hadStoneBefore) {      
            // そのポケットの石を全部取る
    ...
    async function executeMove(start, player) {  
      if (isSunka()) {
        await performSunkaMove(start, player);  
      } else {  
        await performMove(start, player);
      }
    }  
    ...
    async function humanMove(index) {
      ...
      if (busy ||isPaused ||current !== 'human' ||
        !isOwnPit(index, 'human') || boardStones[index].length === 0
      ) {
        return;
      }  
  
      busy = true;   
      await executeMove(index, 'human');  
  
      if (await finishGame()) {return;}  
    ...
    function cloneState(state) {return state.slice();}  
    ...
    function simulateKalaha(state, start, player) {  
      ...
      //ゴール → 追加ターン  
      if (pos === ownGoal(player)) {  
        return {state: s, last: pos, again: true};
      }    
      //横取り  
      if (isOwnPit(pos, player) && s[pos] === 1) {  
        const opposite = oppositePit(pos);  
        s[ownGoal(player)] +=  s[pos] +  s[opposite];  
        s[pos] = 0;
        s[opposite] = 0;
      }    
      return {state: s, last: pos, again: false};
    }  
  
    //Sunka AI simulation   連鎖を再現する。
    ...
    function simulate(state, start, player) {  
      if (isBasic()) {  
        return simulateBasic(state, start, player);
      }    
      if (isKalaha()) {  
        return simulateKalaha(state, start, player);
      }    
      return simulateSunka(state, start, player);
    }  
    ...
    function scoreState(s) {
      ...
      //Basic → 陣地を空に。   Kalaha / Sunka → ゴール数が最重要。
      ...
      const humanPits = s.slice(0, 6).reduce((a, b) => a + b, 0);  
      const cpuPits = s.slice(7, 13).reduce((a, b) => a + b, 0);    
      return ((cpuGoal - humanGoal) * 30 + (humanPits - cpuPits) * 4);
    }  
  
    function evaluateAdvanced(s) {  
      if (isBasic()) {  
        const humanEmpty = s.slice(0, 6).every(v => v === 0);  
        const cpuEmpty = s.slice(7, 13).every(v => v === 0);  
  
        if (cpuEmpty && !humanEmpty) {return -100000;}
      ...
      return ((cpuGoal - humanGoal) * 50 + scoreState(s));
    }  
    ...
    function chooseCpuMove() {  
      ...
      if (cpuLevel === 2) {  
        ...
        for (const m of moves) {  
          ...
          let score = evaluateAdvanced(r.state);  
          ...
          if (score > best) {  
            best = score;  
            candidates = [m];  
          } else if (score === best) {candidates.push(m);}
        }
        ...
        for (const m of moves) {  
          ...
          if (oppMoves.length) {  
            ...
            for (const om of oppMoves) {  
              const or = simulate(r.state, om, 'human');    
              let after = evaluateAdvanced(or.state);  
  
              if (or.again) {after -= 10;}    
      ...
      for (const m of moves) {  
        const result = simulate(board, m,'cpu');    
        let score;  
  
        if (result.again) {  
          score = minimax(result.state, SEARCH_DEPTH - 1, 'cpu', -Infinity, Infinity);  
        } else {  
          score = minimax(result.state, SEARCH_DEPTH - 1, 'human', -Infinity, Infinity);
        }  
  
        if (result.again) {score += 40;}  
    ...
    function minimax( state, depth, player, alpha, beta) {
      ...
      const humanEmpty = state.slice(0, 6).every(v => v === 0);  
      const cpuEmpty = state.slice(7, 13).every(v => v === 0);  
  
      if (humanEmpty || cpuEmpty) {
  
        if (isBasic()) {  
          if (cpuEmpty && !humanEmpty) {return 100000;}  
          if (humanEmpty && !cpuEmpty) {return -100000;}  
        } else {  
          
        //Kalaha / Sunkaでは終了後に残り石がゴールへ入る。  
        //ここではAI評価用にその結果を計算する。
  
          const finalState = finishSimulation(state);  
          const humanScore = finalState[6];  
          const cpuScore = finalState[13];
  
          if (cpuScore > humanScore) {return 100000;}  
          if (humanScore > cpuScore) {return -100000;}
  
          return 0;
        }
      }  
  
      if (depth <= 0) {return evaluateAdvanced(state);}  
  
      const moves = legalMoves( player,state);  
  
      if (!moves.length) {return evaluateAdvanced(state);}  
  
      if (player === 'cpu') {  
        let value = -Infinity;  
  
        for (const move of moves) {  
          const result = simulate( state, move, 'cpu');    
          const nextPlayer =
            result.again
              ? 'cpu'
              : 'human';  
  
          let score = minimax(result.state, depth - 1, nextPlayer, alpha, beta);  
  
          if (result.again) {score += 25;}  
        ...
        for (const move of moves) {  
          const result = simulate( state, move, 'human');    
          const nextPlayer =
            result.again
              ? 'human'
              : 'cpu';    
    ...
    //  AI用の終了時処理 Kalaha / Sunkaでは陣地に残った石をゴールへ移す。
    ...
    function finishSimulation(state) {
      ...
      for (let i = 0; i < 6; i++) {s[i] = 0;}  
      for (let i = 7; i < 13; i++) {s[i] = 0;}  
      return s;
    }  
